...
// مرجع الطلب (Referer) الذي تقبله حماية Bunny CDN — يُقرأ من متغير البيئة
// PUBLIC_API_ORIGIN ويُضبط بعد النشر. عند تركه فارغاً يُحذف الهيدر نهائياً.
const bunnyHeaders = (env: Env): Record<string, string> => {
...
async function handleCheckEncoding(
  ...
  if (isFinished) {
    const cdnHost = env.BUNNY_CDN_HOST || env.VIDEO_CDN_HOST;
    if (!cdnHost) {
      throw new Error('BUNNY_CDN_HOST or VIDEO_CDN_HOST is not configured');
    }
