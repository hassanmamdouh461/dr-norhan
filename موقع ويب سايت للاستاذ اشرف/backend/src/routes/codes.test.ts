...
describe('Activation Codes Redeem Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    await db.prepare(`
      ...
      VALUES ('student-1', 'student-supabase-uid', 'student@fusha.edu.eg', 'student', 'Test Student', 'active', 2, datetime('now'), datetime('now'))
    `).run();

    await db.prepare(`
      INSERT INTO profiles (id, supabase_user_id, email, role, full_name, status, max_devices, created_at, updated_at)
      VALUES ('admin-1', 'admin-supabase-uid', 'admin@fusha.edu.eg', 'admin', 'Test Admin', 'active', 2, datetime('now'), datetime('now'))
    `).run();

    await db.prepare(`
      INSERT INTO courses (id, title, slug, is_published, is_free, is_archived, sort_order, created_at, updated_at)
      VALUES ('course-1', 'Arabic 101', 'arabic-101', 1, 0, 0, 1, datetime('now'), datetime('now'))
    `).run();
    ...
    // Create test JWT token (Cloudflare-native HS256 access token)
