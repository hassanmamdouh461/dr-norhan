...
beforeEach(() => {
  ...
  env = {
    ...
    AUTH_SECRET: 'offline-test-only-signing-key',
  } as Env;
  pending = [];
  errors = [];
...
function subscription(platform = env.PLATFORM_KEY || 'fusha', id = deviceId) {
...
async function expectRecipients(audience: 'user' | 'course' | 'all', tokens: string[], target = 'owner') {
  ...
  expect(db.sqlite.prepare('SELECT * FROM notifications WHERE id = ?').get(result.notification_id))
    .toMatchObject({
      audience, ...message, platform: env.PLATFORM_KEY || 'fusha',
...
function signedJWT(secret = env.AUTH_SECRET) {
  const header = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
  const payload = Buffer.from(JSON.stringify({
    sub: 'owner', email: 'owner@example.test', role: 'student', token_use: 'access',
    iss: 'fusha-api', aud: 'fusha-client',
...
describe.each([
  { label: 'default tenant', configured: undefined, platform: 'fusha', foreign: 'legacy-tenant' },
  { label: 'configured tenant', configured: 'legacy-tenant', platform: 'legacy-tenant', foreign: 'fusha' },
