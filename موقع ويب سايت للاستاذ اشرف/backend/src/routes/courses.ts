...
import { ACCESS_LABELS, canAccess, examCategoryLabel, resolveExamAccess } from '../lib/access';
import { recordMistake, resolveCorrectOptionText } from '../lib/mistakes';
import { awardPoints } from '../lib/points';

const courses = new Hono<HonoBindings>();

/** نقاط إتمام اختبار بنجاح (تُمنح مرة واحدة لكل اختبار). */
const EXAM_COMPLETION_POINTS = 10;

/**
 * عدد الامتحانات المستقلة في حزمة — يُستخدم لتصنيف «خطة ({n} امتحان)»
 * في الوثيقة 23 §3.2.
 */
async function countBundleExams(env: HonoBindings['Bindings'], bundleId: string): Promise<number> {
  ...
  ).bind(bundleId).first<{ count: number }>();
  return row?.count ?? 0;
}
 ...
 */
function buildCorrectAnswerReason(correctOptionText: string, explanation: string | null | undefined): string | null {
...
courses.post('/public-exams/:id/submit', rateLimit('public_exam_submit', 10, 60), async (c) => {
  ...
  let totalScore = 0;
  let earnedScore = 0;
  const gradedQuestions: Record<string, {
    correct: boolean;
    correctOption: string;
    chosenOption: string | null;
    correct_option_text?: string;
    explanation?: string | null;
    correct_answer_reason?: string | null;
  }> = {};
  ...
  for (const q of questions as any[]) {
    ...
    const entry: (typeof gradedQuestions)[string] = {
      correct: isCorrect,
...
courses.get('/exams', requireAuth, async (c) => {
  ...
  const { results: exams } = await c.env.DB.prepare(query).bind(...params).all<any[]>();

  // حالة الوصول لكل امتحان (free / locked / pending / ready)
  const withAccess = await Promise.all((exams as any[]).map(async exam => {
    const access = await resolveExamAccess(c.env, user.id, exam);
    const canOpen = canAccess(access.state);
    const planExamCount = access.via === 'bundle_purchase' && access.bundle_id
      ? await countBundleExams(c.env, access.bundle_id)
      : null;
    return {
      ...exam,
      ...
      can_access: canOpen,
      action_label: canOpen
        ? ACCESS_LABELS.solve_exam
        : access.state === 'pending'
          ? ACCESS_LABELS.awaiting_approval
          : `شراء الامتحان (${exam.price ?? 0} جنيه)`,
...
courses.get('/exams/:id', requireAuth, async (c) => {
  ...
  // حالة الوصول: مجاني / جاهز / قيد المراجعة / مقفل
  const access = await resolveExamAccess(c.env, user.id, exam);
  if (!canAccess(access.state)) {
    return c.json({
      error: {
        code: access.state === 'pending' ? 'PURCHASE_PENDING' : 'EXAM_LOCKED',
        message: access.state === 'pending'
          ? 'طلب شراء هذا الامتحان قيد المراجعة. سيتم تفعيله بعد الموافقة.'
          : 'هذا الامتحان مدفوع. يرجى شراء الامتحان للوصول إليه.',
        access_state: access.state,
        price: access.price,
      },
    }, 403);
...
courses.post('/exams/:id/submit', requireAuth, rateLimit('exam_submit', 10, 60), async (c) => {
  ...
  // حالة الوصول: مجاني / جاهز / قيد المراجعة / مقفل — تُمنع المشاركة لغير المعتمد
  const access = await resolveExamAccess(c.env, user.id, exam);
  if (!canAccess(access.state)) {
    return c.json({
      error: {
  ...
  let totalScore = 0;
  let earnedScore = 0;
  const mistakesToRecord: Array<{ questionId: string; questionText: string | null; givenAnswer: string; correctAnswer: string; correctOptionText: string; pointsLost: number; explanation: string | null }> = [];
  ...
  // كتاب الأخطاء: يُسجَّل كل خطأ تلقائياً
  ...
  // النقاط: تُمنح عند إتمام الامتحان
  const points = await awardPoints(c.env, user.id, EXAM_COMPLETION_POINTS, 'exam_completed', {
    referenceId: exam.id,
  ...
  return c.json({
    ...
    points: { awarded: points.awarded, total_points: points.total_points, level: points.level, level_up: points.level_up },
  });
});
...
courses.get('/:id', optionalAuth, async (c) => {
  ...
  if (user && !isStudentOrGuest) {
    lessonsQuery = `SELECT l.id, l.unit_id, l.title, l.description, l.sort_order, l.is_free_preview,
            ...
            (SELECT status FROM lesson_videos WHERE lesson_id = l.id ORDER BY sort_order ASC, created_at DESC LIMIT 1) as video_status,
            (SELECT provider FROM lesson_videos WHERE lesson_id = l.id ORDER BY sort_order ASC, created_at DESC LIMIT 1) as video_provider,
...
courses.post('/lessons/:id/quiz/submit', requireAuth, rateLimit('quiz_submit', 10, 60), async (c) => {
  ...
  let totalScore = 0;
  let earnedScore = 0;
  const mistakesToRecord: Array<{ questionId: string; questionText: string | null; givenAnswer: string; correctAnswer: string; pointsLost: number }> = [];
  ...
  // النقاط: تُمنح عند إتمام اختبار الدرس
  const points = await awardPoints(c.env, user.id, EXAM_COMPLETION_POINTS, 'exam_completed', {
    referenceId: quiz.id,
