...
export async function generateDirectUploadUrl(
  ...
  const apiOrigin = origin || env.PUBLIC_API_ORIGIN || 'https://api.fusha.site';
  const expires = Math.floor(Date.now() / 1000) + expiresSeconds;
  const secret = env.AUTH_SECRET || 'fusha_upload_fallback_secret';
  const dataToSign = `${objectKey}:${expires}`;
  const signature = await hmacSignHex(utf8Encode(secret), dataToSign);

  return `${apiOrigin}/uploads/direct?key=${encodeURIComponent(objectKey)}&expires=${expires}&sig=${signature}`;
}

export async function verifyDirectUploadSignature(
  secret: string,
  objectKey: string,
  expires: number,
  signature: string
  ...
  const dataToSign = `${objectKey}:${expires}`;
  const expectedSignature = await hmacSignHex(utf8Encode(secret), dataToSign);
...
export async function getPresignedPutUrl(
  ...
  if (!accessKeyId || !secretAccessKey || !accountId) {
    return generateDirectUploadUrl(env, objectKey, expiresSeconds, origin);
  }
  ...
  try {
    ...
    console.warn('[R2_PRESIGN_PUT_FALLBACK] S3 signing failed, falling back to direct upload URL:', err);
    return generateDirectUploadUrl(env, objectKey, expiresSeconds, origin);
  }
}
