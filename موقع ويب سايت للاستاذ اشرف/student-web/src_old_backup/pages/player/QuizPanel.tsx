    ...
    >
      {!user ? (
        ...
        <div className="flex flex-col items-center justify-center" style={{ padding: '30px 0' }}>
          ...
          <p className="body-small" style={{ marginTop: '8px', color: 'rgb(var(--fusha-sand-300))' }}>جاري تحميل الأسئلة...</p>
        </div>
      ) : quizError && !quizData ? (
            ...
            {quizQuestionsList.map((q, idx) => {
              ...
              return (
                <div key={q.id} className="p-3" style={{ backgroundColor: 'rgba(var(--on-surface), 0.01)', borderRadius: '8px', border: '1px solid rgba(var(--on-surface), 0.03)' }}>
                  ...
                  <p style={{ fontSize: '11px', marginTop: '6px', color: studentAnswer === q.correct_option ? 'rgb(var(--success))' : (studentAnswer ? 'rgb(var(--fusha-teal-400))' : 'rgb(var(--error))') }}>
        ...
        <div className="flex flex-col gap-md">
          <div style={{ paddingBottom: '10px', borderBottom: '1px solid rgb(var(--outline) / 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontSize: '11px', color: "rgb(var(--on-surface-variant))", marginTop: '2px' }}>الدرجة الكلية: {quizData.max_score} درجة • الأسئلة: {quizQuestionsList.length}</p>
            </div>
            {quizTimeLeft !== null && (
