...
// المضيف المسموح يُقرأ من متغير البيئة R2_PUBLIC_HOST (يُضبط بعد النشر)
// ويجب أن يطابق VITE_R2_PUBLIC_HOST في src/utils/img.ts.
const MIN_WIDTH = 16;
const MAX_WIDTH = 1600;

export const onRequestGet: PagesFunction<{ IMAGES: unknown; R2_PUBLIC_HOST?: string }> = async (context) => {
  ...
  // لم يُضبط مضيف R2 بعد النشر — لا يُسمح بأي مضيف.
  const allowedHosts = new Set(env.R2_PUBLIC_HOST ? [env.R2_PUBLIC_HOST] : []);
  if (srcUrl.protocol !== 'https:' || !allowedHosts.has(srcUrl.hostname)) {
