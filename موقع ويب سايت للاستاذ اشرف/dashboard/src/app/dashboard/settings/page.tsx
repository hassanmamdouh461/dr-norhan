...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const fetchProfileAndSettings = async () => {
      try {
        setLoading(true);
        // GET /auth/me يُعيد { user } — نفكّ الغلاف قبل الاستهلاك.
  ...
  const handleUpdateName = async (e: React.FormEvent) => {
    ...
    try {
      // PATCH /auth/me يُعيد { user } كذلك.
      const patchResponse = await apiPatch<any>('/auth/me', { full_name: fullName.trim() });
      const updated = patchResponse?.user ?? patchResponse;
      if (updated) {
        setProfile(updated);
        setStatus({ type: 'success', message: 'تم تحديث الاسم بنجاح!' });
        
        // Sync local storage session
        const cached = localStorage.getItem('fusha_dashboard_session');
        if (cached) {
          const parsed = JSON.parse(cached);
          parsed.user.full_name = updated.full_name;
          localStorage.setItem('fusha_dashboard_session', JSON.stringify(parsed));
        }
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-ambient p-6 md:p-8">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6 p-6 rounded-xl bg-surface-container-low border border-outline-variant/40 mb-8">
          <div className="relative group cursor-pointer" onClick={triggerFileSelect}>
            <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-primary/40 shadow-ambient bg-surface flex items-center justify-center">
              <img 
          ...
          <div className="text-center sm:text-start space-y-2 flex-1">
            ...
            <button 
              ...
              disabled={uploading}
              className="px-4 py-2 border border-outline-variant hover:bg-surface text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 mx-auto sm:ms-0 disabled:opacity-50"
            >
              <Upload size={14} />
        ...
        <form onSubmit={handleUpdateName} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-on-surface ms-1 flex items-center gap-1.5">
            ...
            <div className="space-y-2">
              ...
              <div className="w-full px-4 py-3 border border-outline-variant/60 rounded-xl bg-surface-container/50 text-slate-400 text-sm flex items-center cursor-not-allowed select-all">
                {profile?.email || 'admin@fusha.edu.eg'}
              </div>
            </div>
