...
import { Course, Unit, Lesson, DeleteTarget, CourseFormData, NewUnitState, NewLessonState, VideoProgressState } from './types';
...
export default function CoursesPage() {
  ...
  const [videoProcessingMap, setVideoProcessingMap] = useState<Record<string, VideoProgressState>>({});
  ...
  const handleProcessingStarted = useCallback((lessonId: string) => {
    setVideoProcessingMap(prev => ({
      ...prev,
      [lessonId]: {
        status: 'processing',
        percent: 5,
        text: 'جاري المعالجة السحابية (5%)',
      },
    }));
    const courseId = getCourseIdForLesson(lessonId);
    if (courseId) loadCourseContents(courseId);
  }, [getCourseIdForLesson, loadCourseContents]);

  // Background polling for videos being transcoded/transferred
  useEffect(() => {
    const processingLessonIds = new Set<string>();

    for (const [id, state] of Object.entries(videoProcessingMap)) {
      if (state.status === 'processing') {
        processingLessonIds.add(id);
      }
    }
    ...
    for (const lessonList of Object.values(lessons)) {
      for (const l of lessonList) {
        if (
          l.video_status === 'processing' &&
          videoProcessingMap[l.id]?.status !== 'ready' &&
          videoProcessingMap[l.id]?.status !== 'error'
        ) {
          processingLessonIds.add(l.id);
        }
      }
    }

    if (processingLessonIds.size === 0) return;

    let cancelled = false;

    const poll = async () => {
      for (const lessonId of Array.from(processingLessonIds)) {
        if (cancelled) break;
        try {
          const statusResp = await api(`/admin/lessons/${lessonId}/videos/transfer-status`) as {
            status: string;
          ...
          if (statusResp.status === 'ready') {
            ...
            setVideoProcessingMap(prev => ({
              ...
              [lessonId]: {
                status: 'error',
                percent: 0,
                text: 'فشل المعالجة السحابية',
              },
            }));
          } else if (statusResp.status === 'processing') {
            let overallPercent = 5;
            if (statusResp.transfer_progress !== undefined && statusResp.transfer_progress !== null) {
              overallPercent = 75 + Math.round((statusResp.transfer_progress / 100) * 20);
            } else if (statusResp.transcoding_progress !== undefined && statusResp.transcoding_progress !== null) {
              overallPercent = Math.round((statusResp.transcoding_progress / 100) * 75);
            } else if (statusResp.completed_qualities && statusResp.completed_qualities.length > 0) {
              const totalQ = statusResp.total_qualities || 3;
              overallPercent = 75 + Math.round((statusResp.completed_qualities.length / totalQ) * 20);
            } else {
              const prevPercent = videoProcessingMap[lessonId]?.percent || 5;
              overallPercent = Math.min(prevPercent + 2, 70);
            }
            ...
            setVideoProcessingMap(prev => ({
              ...
              [lessonId]: {
                status: 'processing',
                percent: Math.max(overallPercent, prev[lessonId]?.percent || 5),
                text: `معالجة سحابية (${overallPercent}%)`,
              },
            }));
          }
        } catch {
  ...
  }, [videoProcessingMap, lessons, getCourseIdForLesson, loadCourseContents]);
  ...
  return (
    <div className="space-y-6 text-on-surface">
      ...
      <CourseTree
        ...
        videoProcessingMap={videoProcessingMap}
      />
