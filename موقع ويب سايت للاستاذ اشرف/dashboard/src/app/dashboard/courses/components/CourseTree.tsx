...
import { Course, Unit, Lesson, NewUnitState, NewLessonState, VideoProgressState } from '../types';
...
interface CourseTreeProps {
  ...
  videoProcessingMap?: Record<string, VideoProgressState>;
    ...
    <div className="space-y-4">
      {loading ? (
        ...
        <div className="p-8 sm:p-12 text-center bg-surface border border-outline-variant rounded-xl text-on-surface-variant">
          ...
          <div className="mx-auto mt-6 grid max-w-2xl grid-cols-1 gap-2 text-start sm:grid-cols-3"><span className="rounded-lg bg-surface-container-low p-3 text-xs font-bold text-on-surface"><b className="text-primary">1.</b> بيانات الدورة</span><span className="rounded-lg bg-surface-container-low p-3 text-xs font-bold text-on-surface"><b className="text-primary">2.</b> الوحدات والمحاضرات</span><span className="rounded-lg bg-surface-container-low p-3 text-xs font-bold text-on-surface"><b className="text-primary">3.</b> مراجعة ونشر</span></div>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex flex-col text-end font-mono text-[10px] text-on-surface-variant">
                        ...
                        <div className="space-y-2">
                          {lessons[unit.id] && lessons[unit.id].length > 0 ? (
                            lessons[unit.id].map(lesson => {
                              const progressState = videoProcessingMap?.[lesson.id];
                              const isProcessing = progressState?.status === 'processing' || (lesson.video_status === 'processing' && progressState?.status !== 'ready' && progressState?.status !== 'error');
                              const currentPercent = progressState?.status === 'processing' ? progressState.percent : (lesson.video_status === 'processing' ? 15 : 0);
                              const isReady = progressState?.status === 'ready' || lesson.video_status === 'ready';
                              const isError = progressState?.status === 'error' || lesson.video_status === 'error';

                              return (
                                <div
                                  key={lesson.id}
                                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3.5 bg-surface-container-lowest border border-outline-variant/60 rounded-lg hover:border-primary/30 transition duration-150"
                                >
                                  <div className="flex items-center gap-3 min-w-0">
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border ${
                                      isProcessing
                                        ...
                                        : 'bg-surface-container-low border-outline-variant/60 text-secondary'
                                    }`}>
                                      <span className={`material-symbols-outlined text-base ${isProcessing ? 'animate-spin' : ''}`}>
                                        {isProcessing ? 'sync' : isReady ? 'videocam' : 'videocam'}
                                      </span>
                                    </div>
                                    <div className="min-w-0 flex-1">
                                      <div className="flex items-center gap-2 flex-wrap">
                                        <p className="text-xs font-bold text-on-surface truncate">{lesson.title}</p>

                                        {/* Dynamic Video Processing & Status Badges */}
                                        {isProcessing && (
                                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/30 shadow-xs animate-pulse">
                                            <span className="material-symbols-outlined text-[12px] animate-spin">progress_activity</span>
                                            <span>معالجة سحابية: {currentPercent}%</span>
                                          </span>
                                        )}
                                        ...
                                        {isError && !isProcessing && (
                                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-error-container/20 text-error border border-error/30">
                                            <span className="material-symbols-outlined text-[12px]">error</span>
                                            <span>فشل المعالجة السحابية</span>
                                          </span>
                                        )}
                                      </div>
                                      ...
                                      <div className="flex items-center gap-2 mt-0.5">
                                        ...
                                        {lesson.duration_seconds && lesson.duration_seconds > 0 ? (
                                          <span className="text-[10px] text-on-surface-variant/80 font-mono">
                                            • {Math.floor(lesson.duration_seconds / 60)}:{(lesson.duration_seconds % 60).toString().padStart(2, '0')} دقيقة
                                          </span>
                                        ) : null}
                                      </div>

                                      {/* Micro progress bar under title when processing */}
                                      {isProcessing && (
                                        <div className="w-44 max-w-full bg-amber-500/20 h-1.5 rounded-full overflow-hidden mt-1.5 shadow-inner">
                                          <div
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0">
                                    ...
                                    <button
                                      ...
                                      className={`flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold rounded-lg transition border ${
                                        isProcessing
                                    ...
                                    >
                                      <span className={`material-symbols-outlined text-xs ${isProcessing ? 'animate-spin' : ''}`}>
                                        {isProcessing ? 'sync' : isReady ? 'play_circle' : 'cloud_upload'}
                                      </span>
                                      <span>{isProcessing ? `معالجة (${currentPercent}%)` : isReady ? 'تعديل الفيديو' : 'رفع الفيديوهات'}</span>
                                    </button>
                                    <button
                          ...
                          <div className="border border-dashed border-outline-variant rounded-lg p-3 bg-surface-container-low/50 flex flex-col sm:flex-row items-center gap-3 pt-3">
                            <input
                              ...
                              className="w-full sm:flex-1 px-3 py-1.5 bg-surface border border-outline-variant rounded-lg text-xs outline-none focus:border-primary text-start"
                            />
                ...
                <div className="bg-surface border border-outline-variant rounded-xl p-5 space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-2 bg-surface-container-low border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-lg text-xs outline-none transition text-start"
                    />
                    <button
