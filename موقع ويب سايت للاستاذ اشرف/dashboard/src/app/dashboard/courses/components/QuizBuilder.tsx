...
export default function QuizBuilder({ lessonId, isOpen, onClose }: QuizBuilderProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-6 max-w-4xl w-full space-y-6 shadow-ambient animate-in zoom-in-95 duration-200 my-8 max-h-[90vh] overflow-y-auto text-start"
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-6 space-y-5">
            ...
            <div className="space-y-3">
              ...
              {!quizId ? (
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto ps-1">
                  {quizQuestions.map((q, idx) => (
                    <div key={q.id} className="p-3 bg-surface-container-low hover:bg-surface-container border border-outline-variant/60 rounded-xl flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <p className="text-[11px] font-bold text-on-surface text-start">س {idx + 1}: {q.question_text || '(سؤال بصورة فقط)'}</p>
                        {q.image_url && (
          ...
          <div className="md:col-span-6 border-s md:border-s border-outline-variant ps-0 md:ps-6 space-y-4">
            <h4 className="text-xs font-bold text-on-surface flex items-center gap-1.5 justify-end">
                  ...
                  {questionImageUrl && (
                    <div className="relative group">
                      ...
                      <button
                        ...
                        className="absolute -top-1.5 -end-1.5 bg-error text-on-error rounded-full w-4 h-4 flex items-center justify-center text-[10px]"
                      >
                        ×
                      </button>
                    </div>
                  ...
                  {questionOptions.map((opt, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 px-3 py-2 text-xs border border-outline-variant rounded-xl focus:border-primary outline-none text-start placeholder-outline font-semibold"
                      />
                      <input
                        type="radio"
