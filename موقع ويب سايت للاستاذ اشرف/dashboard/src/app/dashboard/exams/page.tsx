      ...
      {activeTab === 'manage' ? (
        ...
        selectedExamId === null ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {exams.map(exam => (
                    <button
                      ...
                      className="group bg-surface border border-outline-variant p-5 rounded-2xl hover:border-primary/30 hover:shadow-md transition cursor-pointer disabled:cursor-wait disabled:opacity-60 text-start flex flex-col justify-between"
                    >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-sidebar border border-outline-variant/30 text-white p-6 rounded-2xl shadow-lg flex justify-between items-center flex-wrap gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    onClick={() => beginExamSelection(null)}
                    className="flex items-center gap-1 text-xs font-bold bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg transition"
                  >
                ...
                <h2 className="text-lg font-black font-cairo mt-2 text-white">{examTitle}</h2>
              </div>

              <div className="flex items-center gap-3">
                <div className="text-end bg-black/20 px-4 py-2 rounded-xl border border-white/10">
                  <span className="text-[10px] text-emerald-100/80 block font-semibold">أسئلة الامتحان</span>
                  <strong className="text-md font-black text-white">{questions.length} أسئلة</strong>
                </div>
                <div className="text-end bg-black/20 px-4 py-2 rounded-xl border border-white/10">
                  <span className="text-[10px] text-emerald-100/80 block font-semibold">الدرجة الكلية</span>
                  <strong className="text-md font-black text-white">{examMaxScore} درجة</strong>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      {questionImageUrl && (
                        <div className="mt-2 relative inline-block">
                          ...
                          <button
                            ...
                            className="absolute -top-1.5 -end-1.5 bg-error text-on-error rounded-full w-4.5 h-4.5 flex items-center justify-center shadow-md hover:bg-error/95 text-[10px]"
                          >
                            ×
                          </button>
                        </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="space-y-3 max-h-[750px] overflow-y-auto ps-1">
                  {questions.length === 0 ? (
                    <div className="bg-surface border border-dashed border-outline-variantp-12 rounded-2xl text-center">
            ...
            <table className="w-full text-start border-collapse">
              <thead>
                <tr className="bg-surface-container-low text-on-background font-bold text-xs border-b border-outline-variant">
                  ...
                  {gradeExams.map(exam => (
                    <th key={exam.id} className="p-4 text-center font-bold border-s border-outline-variant/50">
                      {exam.title}
                  ...
                  filteredGrades.map(student => (
                    <tr key={student.student_id} className="hover:bg-surface-container-low/50 transition-colors">
                      ...
                      {gradeExams.map(exam => {
                        ...
                        return (
                          <td key={exam.id} className="p-4 text-center border-s border-outline-variant/30">
                            {hasScore ? (
