  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!session) {
        ...
        const name = session.user.user_metadata?.full_name || 'محمد حمدي';
        ...
        try {
          ...
          if (profile) {
            if (!['admin', 'assistant'].includes(profile.role)) {
              console.error("[AUTH] Access denied: User is not staff");
            ...
            const userData = { email, role: profile.role, full_name: profile.full_name || name, avatar_url: avatarUrl };
            setUser(userData);
            localStorage.setItem('chemistry_dashboard_session', JSON.stringify({ user: userData, token: session.access_token }));
        ...
        const userData = { email, role, full_name: name, avatar_url: avatarUrl };
        setUser(userData);
        localStorage.setItem('chemistry_dashboard_session', JSON.stringify({ user: userData, token: session.access_token }));
      }
      setLoading(false);
    ...
    <div className="min-h-screen bg-background text-on-background flex flex-col md:flex-row rtl font-body-md">
      ...
      <nav className="md:hidden flex flex-row-reverse items-center justify-between px-4 w-full sticky top-0 z-50 backdrop-blur-md bg-surface border-b border-outline-variant h-16">
        <div className="flex min-w-0 items-center gap-2">
          ...
          />
          <span className="truncate font-headline-sm text-base font-bold text-primary">الهضبة</span>
        </div>
      ...
      {/* Desktop Side Nav */}
      <aside className="hidden md:flex flex-col h-screen w-64 fixed right-0 top-0 bg-surface-container-low border-l border-outline-variant py-6 z-40">
        <div className="px-md mb-lg flex flex-col items-center">
          <a 
            href="https://www.synapticstudio.tech/ar" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="flex h-16 w-16 mb-sm items-center justify-center rounded-2xl bg-gradient-to-tr from-teal-700 to-teal-600 font-extrabold text-white text-2xl shadow-lg border border-teal-500/20 hover:opacity-95 transition-opacity cursor-pointer tracking-wider"
            title="Synaptic Studio"
          >
            SS
          </a>
          <h2 className="font-headline-sm text-headline-sm text-on-surface text-center">لوحة تحكم الهضبة</h2>
        ...
        <div className="px-sm mt-auto space-y-1">
          ...
          <button
            ...
            className="w-full flex flex-row-reverse items-center justify-end gap-sm text-error px-4 py-3 font-label-md text-label-md hover:bg-error-container/20 rounded-lg transition-all duration-200 ease-in-out text-right"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="block text-center text-[11px] text-secondary hover:text-primary transition-colors duration-200 pt-4 mt-auto border-t border-outline-variant"
          >
            جميع حقوق النشر والتشغيل محفوظة لـ سينابتك ستوديو
          </a>
        </div>
      </aside>
        ...
        <div className="fixed inset-0 z-50 md:hidden flex justify-start">
          ...
          <aside className="relative w-72 max-w-[88vw] bg-surface-container-low h-full flex flex-col py-6 px-3 z-50 shadow-2xl animate-in slide-in-from-right duration-200">
            <button 
              ...
              className="absolute left-4 top-4 p-2 text-on-surface-variant hover:bg-surface-container-high rounded-full flex items-center"
            >
            ...
            <div className="px-md mb-lg mt-6 flex flex-col items-center">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="flex h-14 w-14 mb-sm items-center justify-center rounded-xl bg-gradient-to-tr from-teal-700 to-teal-600 font-extrabold text-white text-xl shadow-lg border border-teal-500/20 hover:opacity-95 transition-opacity cursor-pointer tracking-wider"
                title="Synaptic Studio"
              >
                SS
              </a>
              <h2 className="font-headline-sm text-[16px] text-on-surface text-center">لوحة تحكم الهضبة</h2>
      ...
      {/* Main Content Area */}
      <div className="flex-1 md:mr-64 flex flex-col min-h-screen relative w-full font-body-md text-body-md bg-background text-on-background">
        ...
        <header className="hidden md:flex flex-row-reverse items-center justify-between px-8 lg:px-12 w-full h-20 sticky top-0 z-30 bg-background/90 backdrop-blur-sm border-b border-outline-variant/30">
          ...
          <div className="flex items-center gap-sm text-on-surface-variant">
            ...
            <img 
              className="w-10 h-10 rounded-full mr-4 object-cover border border-outline-variant" 
              src={user?.avatar_url || '/images/default_avatar.png'} 
              alt="Teacher profile"
            />
          </div>
        ...
        {/* Mobile Bottom Nav */}
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-surface/95 backdrop-blur-lg border-t border-outline-variant flex justify-around py-2.5 shadow-lg">
