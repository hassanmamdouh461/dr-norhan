  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      >
        ...
        {error && (
          <div role="alert" className="p-3 rounded-lg bg-error-container/20 border border-error-container text-error text-xs text-right">
            {error}
          </div>
        )}

        <fieldset disabled={busy} className="space-y-4 text-right">
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-on-surface-variant mr-1">اسم الفصل الدراسي</label>
            <input
              placeholder={isCreate ? 'مثال: كورس الكيمياء غير العضوية والتحليلية...' : 'اسم الفصل الدراسي...'}
              value={formData.title}
              onChange={e => setFormData({ ...formData, title: e.target.value })}
              className="w-full px-4 py-2.5 bg-surface-container-low border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-surface placeholder:text-outline focus:outline-none transition text-right"
            />
          </div>

          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-on-surface-variant mr-1">المرحلة الدراسية / الصف</label>
            <select
              value={formData.grade}
              onChange={e => setFormData({ ...formData, grade: e.target.value })}
              className="w-full px-4 py-2.5 bg-surface-container-low border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-surface focus:outline-none transition text-right"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-on-surface-variant mr-1">الشعبة الدراسية / المسار</label>
            <select
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-bold text-on-surface-variant mr-1">الوصف التعريفي للفصل الدراسي</label>
            <textarea
              ...
              className="w-full px-4 py-2.5 bg-surface-container-low border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-surface placeholder:text-outline focus:outline-none transition resize-none text-right"
            />
          </div>

          {/* Cover Image Uploader with simulation and alignment dragging */}
          <div className="space-y-3">
            <label className="text-[11px] font-bold text-on-surface-variant mr-1 block">صورة غلاف الفصل الدراسي</label>
            
            {/* Preferred Dimensions Notice */}
            <div className="text-[10px] bg-primary/5 text-primary border border-primary/10 rounded-xl p-2.5 leading-relaxed text-right font-medium">
            ...
            {formData.cover_url ? (
              <div className="space-y-3">
                ...
                <div className="border border-outline-variant rounded-xl p-3 bg-surface-container-low space-y-3 text-right">
                  ...
                  <div className="space-y-2 pt-2 border-t border-outline-variant/30 text-[10px]">
                    <div className="flex items-center gap-2">
                      <span className="text-on-surface-variant min-w-[85px] text-right font-medium">تكبير/تصغير ({parsedZoom}%)</span>
                      <input 
                        type="range" 
                        min="100" 
                        max="250" 
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-on-surface-variant min-w-[85px] text-right font-medium">أفقي ({parsedX}%)</span>
                      <input 
                        type="range" 
                        min="0" 
                        max="100" 
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-on-surface-variant min-w-[85px] text-right font-medium">رأسي ({parsedY}%)</span>
                      <input 
                        type="range" 
                        min="0" 
                        max="100" 
          ...
          <label className="flex items-center gap-2 select-none cursor-pointer p-1.5 border border-outline-variant rounded-xl bg-surface-container-low/50 justify-start">
            ...
            />
            <span className="text-xs text-on-surface font-semibold mr-1">
          ...
          {!formData.is_free && (
            <div className="space-y-1 text-right">
              <label className="text-[11px] font-bold text-on-surface-variant mr-1">سعر الكورس (بالجنيه المصري)</label>
              <input
                type="number"
                value={formData.reference_price || ''}
                onChange={e => setFormData({ ...formData, reference_price: parseInt(e.target.value) || 0 })}
                placeholder="مثال: 150"
                className="w-full px-3.5 py-2 text-xs border border-outline-variant rounded-xl focus:outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/5 text-right font-medium"
              />
            </div>
          )}
        </fieldset>
