...
export default function VideoUploadPanel({ lessonId, isOpen, onClose, onUploaded, setGlobalError }: VideoUploadPanelProps) {
  ...
  const [videoSourceTab, setVideoSourceTab] = useState<'bunny' | 'youtube' | 'r2'>('r2');
  ...
  const retryVideoTransfer = async () => {
    ...
    try {
      ...
      setTransferIssue(null);
      setGlobalError('');
      setUploadProgress('تم إعادة جدولة النقل، جاري معالجة الفيديو ونقله سحابياً... (5%)');
      startBunnyTransferPolling(lessonId, transferIssue.bunnyGuid);
    } catch (e: any) {
  ...
  const uploadBunnyVideo = async (file: File) => {
    ...
    try {
      ...
      setUploadProgress('جاري بدء المعالجة والترميز السحابي...');
      await apiPost(`/admin/lessons/${lessonId}/videos/bunny-upload-complete`, {
        video_id,
        bunny_guid
      });

      // 4. Start polling for transfer status
      setUploadProgress('جاري معالجة الفيديو ونقله سحابياً... (5%)');
      startBunnyTransferPolling(lessonId, bunny_guid);
      ...
      >
        ...
        {isUploading ? (
          ...
          <div className="space-y-6 py-4 text-center select-none animate-in fade-in duration-300">
            ...
            {videoSourceTab === 'r2' && (
              <div className="grid grid-cols-2 gap-3 bg-surface-container-low border border-outline-variant/30 rounded-xl p-3 text-right">
                ...
                <div className="space-y-1 border-l border-outline-variant pl-2">
            ...
            {getProgressPercent() > 0 ? (
              ...
              <div className="w-full bg-surface-container h-2.5 rounded-full overflow-hidden border border-outline-variant/30 relative">
                <div className="bg-primary h-full w-1/2 rounded-full absolute left-1/4 animate-pulse" />
              </div>
            )}

            {/* Individual quality progress tracking - Only for R2 HLS */}
            {videoSourceTab === 'r2' && detailedProgress && detailedProgress.qualities && (
              <div className="bg-surface-container-low border border-outline-variant/40 rounded-xl p-3.5 space-y-2.5 text-right select-none">
            ...
            {videoSourceTab === 'r2' && pipelineLogs.length > 0 && (
              <div className="bg-[#111827] border border-slate-800 rounded-xl p-3 text-right">
                <button
                  type="button"
                ...
                {showUploadLogs && (
                  <div className="mt-2.5">
                    ...
                    <div
                      className="h-36 overflow-y-auto bg-slate-950 rounded-lg p-2.5 font-mono text-[9px] text-left leading-relaxed scrollbar-thin select-text"
                      style={{ direction: 'ltr' }}
                    >
          ...
          <div className="space-y-5">
            {videoSourceTab === 'youtube' ? (
              ...
              <div className="space-y-4 text-right animate-in fade-in duration-200">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-on-surface-variant mr-1">رابط أو معرف فيديو اليوتيوب</label>
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3 py-2.5 text-xs border border-outline-variant rounded-xl focus:border-primary focus:ring-1 focus:ring-primary outline-none pr-8 text-left"
                      dir="ltr"
                    />
                    <span className="material-symbols-outlined absolute right-2.5 top-3 text-outline text-sm">link</span>
                  </div>
              ...
              <div className="space-y-4 text-right animate-in fade-in duration-200">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-on-surface-variant mr-1">
                ...
                <div className="p-3 bg-warning-container/30 border border-warning-container rounded-xl space-y-1">
                  ...
                  <ul className="text-[9px] text-on-warning-container leading-relaxed list-disc pr-3">
                    {videoSourceTab === 'bunny' ? (
                      <>
              ...
              <div className="space-y-4 text-right animate-in fade-in duration-200">
                ...
                {videoSourceTab === 'r2' ? (
                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-on-surface-variant mr-1 block">اختر جودات معالجة البث المطلوبة (HLS):</label>
                  ...
                  <div className="p-3.5 bg-primary/5 border border-primary/10 rounded-xl space-y-1.5 text-right select-none animate-in fade-in duration-200">
                ...
                {videoSourceTab === 'r2' && selectedQualities.some(q => {
                  ...
                  <div className="p-3 bg-warning-container/40 border border-warning-container rounded-xl space-y-1 text-right animate-in slide-in-from-top-1 duration-200">
