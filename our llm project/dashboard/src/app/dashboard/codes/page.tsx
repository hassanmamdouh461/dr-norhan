...
export default function CodesPage() {
  ...
  const downloadCSV = (batchId: string) => {
    window.open(`${process.env.NEXT_PUBLIC_API_URL || 'https://alhadaba-chemistry-api.lolelarap.workers.dev'}/admin/codes/batches/${batchId}/csv`, '_blank');
  };
    ...
    <div className="space-y-8 select-none animate-in fade-in duration-300">
      ...
      {generatedCodes.length > 0 && (
        <div className="bg-emerald-50/50 border border-emerald-200 rounded-2xl p-6 space-y-4 shadow-ambient animate-in slide-in-from-top-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-2 max-h-52 overflow-y-auto p-3 bg-white rounded-xl border border-outline-variant/60">
            {generatedCodes.map(code => (
              <button 
                key={code} 
                onClick={() => copyCode(code)}
                className="px-3 py-2.5 bg-slate-50 hover:bg-primary/5 border border-outline-variant/50 hover:border-primary/30 rounded-lg text-slate-800 font-mono text-xs hover:scale-[1.02] active:scale-[0.98] transition-all text-center flex items-center justify-between"
              >
                <span>{code}</span>
                <span className="material-symbols-outlined text-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Total Codes */}
        <div className="bg-white border border-outline-variant rounded-xl p-6 hover:shadow-ambient transition-shadow">
          <div className="flex justify-between items-start mb-4">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
              <span className="material-symbols-outlined text-xl">description</span>
            </div>
            <span className="px-2 py-1 bg-surface-container-high rounded text-secondary font-label-md text-label-md">إجمالي المولد</span>
          </div>
          <p className="font-body-sm text-body-sm text-secondary mb-1">إجمالي الرموز النشطة</p>
        ...
        <div className="bg-white border border-outline-variant rounded-xl p-6 hover:shadow-ambient transition-shadow">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="px-2 py-1 bg-surface-container-high rounded text-secondary font-label-md text-label-md">متاح للاستخدام</span>
          </div>
          <p className="font-body-sm text-body-sm text-secondary mb-1">رموز غير مستخدمة</p>
        ...
        <div className="bg-white border border-outline-variant rounded-xl p-6 hover:shadow-ambient transition-shadow">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="px-2 py-1 bg-surface-container-high rounded text-secondary font-label-md text-label-md">تاريخي</span>
          </div>
          <p className="font-body-sm text-body-sm text-secondary mb-1">رموز تم استخدامها</p>
          <p className="font-display-lg text-[32px] font-bold text-on-background">{totalUsed.toLocaleString('ar-EG')}</p>
        </div>
      </div>

      {/* Batches Section */}
      <div className="bg-white border border-outline-variant rounded-xl overflow-hidden shadow-ambient">
        <div className="p-6 border-b border-outline-variant flex flex-col sm:flex-row justify-between items-start sm:items-center bg-surface-container-low/50 gap-4">
          ...
          <div className="relative w-full sm:w-64">
            <input 
              ...
              className="w-full pl-4 pr-10 py-2 bg-white border border-outline-variant rounded-lg font-body-sm text-body-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all text-right" 
              placeholder="البحث في الدفعات..." 
              type="text"
            />
            <span className="material-symbols-outlined absolute right-3 top-2.5 text-secondary text-base">search</span>
          </div>
        </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredBatches.map(batch => {
                  ...
                  return (
                    <div 
                      key={batch.id} 
                      className="border border-outline-variant rounded-lg p-5 hover:shadow-ambient transition-shadow bg-white relative overflow-hidden group flex flex-col justify-between"
                    >
                      {/* Decorator line */}
                      <div className={`absolute right-0 top-0 bottom-0 w-1 ${borderColorClass}`} />
                          ...
                          <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div 
                              className={`h-full rounded-full transition-all duration-500 ${
                                isCompleted ? 'bg-red-500' : 'bg-gradient-to-l from-primary to-emerald-400'
                              }`} 
                      ...
                      <div className="space-y-4">
                        {batch.note && (
                          <p className="text-[11px] text-secondary italic bg-slate-50 p-2 rounded border border-outline-variant/30 truncate" title={batch.note}>
                            {batch.note}
                          </p>
                        )}
                        <div className="flex justify-between items-center pt-3 border-t border-outline-variant/60">
                          ...
                          <div className="flex items-center gap-1">
                            <button 
                              ...
                              className={`p-1.5 rounded-md transition-all flex items-center justify-center ${
                                showCodes === batch.id 
                                  ...
                                  : 'text-secondary hover:text-primary hover:bg-slate-50 border border-transparent'
                              }`}
                            >
                            ...
                            <button 
                              ...
                              className="p-1.5 text-secondary hover:text-primary hover:bg-slate-50 rounded-md border border-transparent transition flex items-center justify-center"
                            >
                              <span className="material-symbols-outlined text-base">download</span>
                            </button>
                            <button 
                              ...
                              className="p-1.5 text-secondary hover:text-red-600 hover:bg-red-50 rounded-md border border-transparent transition flex items-center justify-center"
                            >
                            ...
                            <div className="max-h-48 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                              {batchCodes.length > 0 ? (
                                batchCodes.map(code => (
                                  <div 
                                    key={code.code} 
                                    className="flex items-center justify-between bg-slate-50 border border-outline-variant/40 rounded-lg px-3 py-2"
                                  >
                                    <div className="flex items-center gap-2">
                                      <button 
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div 
            className="bg-white border border-outline-variant rounded-2xl p-6 max-w-lg w-full space-y-6 shadow-2xl animate-in zoom-in-95 duration-200 text-on-surface" 
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-outline-variant/50 pb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-on-background">معالج توليد الأكواد</h3>
              </div>
              <button 
                onClick={() => setShowCreate(false)}
                className="p-1.5 text-secondary hover:bg-slate-100 rounded-full transition-colors flex items-center justify-center"
              >
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-slate-50 rounded-xl border border-outline-variant/40">
              {[
                ...
                <React.Fragment key={s.step}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[11px] font-bold transition-colors ${
                      wizardStep === s.step ? 'text-primary' : 'text-secondary'
                    }`}>
                      {s.label}
                    </span>
                  </div>
                  {idx < arr.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-2 ${
                      wizardStep > s.step ? 'bg-primary' : 'bg-slate-200'
                    }`} />
                  )}
                </React.Fragment>
              ))}
            ...
            <div className="space-y-4 min-h-[180px]">
              ...
              {wizardStep === 1 && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-secondary mr-1">اسم دفعة الأكواد</label>
                    <input 
                      placeholder="مثال: العناصر الانتقالية - سنتر الأمل (يوليو)" 
                      value={form.name} 
                      onChange={e => setForm({ ...form, name: e.target.value })}
                      className="w-full px-4 py-2.5 bg-white border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-background placeholder-secondary/40 focus:outline-none transition" 
                    />
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-secondary mr-1">نطاق عمل الأكواد</label>
                    <select 
                      value={form.scope_type} 
                      onChange={e => setForm({ ...form, scope_type: e.target.value, course_id: '' })}
                      className="w-full px-4 py-2.5 bg-white border border-outline-variant focus:border-primary rounded-xl text-xs text-on-background focus:outline-none transition"
                    >
                  ...
                  {form.scope_type === 'course' && (
                    <div className="space-y-1.5 animate-in slide-in-from-top-1">
                      <label className="text-[11px] font-bold text-secondary mr-1">اختر الكورس المستهدف</label>
                      <select 
              ...
              {wizardStep === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-200">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-secondary mr-1">عدد الأكواد المطلوبة</label>
                    <input 
                      type="number" 
                      min={1}
                      max={500}
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-background placeholder-secondary/40 focus:outline-none transition font-mono" 
                    />
                    <span className="text-[9px] text-secondary block">الحد الأقصى للتوليد دفعة واحدة هو ٥٠٠ كود.</span>
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-secondary mr-1">صلاحية الكود بعد التفعيل (بالأيام)</label>
                    <input 
                      type="number" 
                      min={1}
                      ...
                      className="w-full px-4 py-2.5 bg-white border border-outline-variant focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-xs text-on-background placeholder-secondary/40 focus:outline-none transition font-mono" 
                    />
                    <span className="text-[9px] text-secondary block">عدد الأيام المتاحة للطالب لمشاهدة الكورس بعد إدخال الكود.</span>
                  </div>
                </div>
              )}
              ...
              {wizardStep === 3 && (
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold text-secondary mr-1">ملاحظة الطباعة (تطبع على ورقة الكود)</label>
                    <input 
                  ...
                  {/* Summary Card */}
                  <div className="p-4 rounded-xl bg-slate-50 border border-outline-variant/60 space-y-2.5">
                    <h4 className="text-[11px] font-black text-secondary uppercase tracking-wider">ملخص الدفعة الجاري توليدها:</h4>
                    <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                      <div>
                        <span className="text-secondary block text-[10px]">اسم الدفعة:</span>
                        <span className="font-bold text-on-background">{form.name}</span>
                      </div>
                      <div>
                        <span className="text-secondary block text-[10px]">الكورس المستهدف:</span>
                      ...
                      <div>
                        <span className="text-secondary block text-[10px]">الكمية المطلوبة:</span>
                        <span className="font-bold font-mono text-on-background">{form.quantity} كود تفعيل</span>
                      </div>
                      <div>
                        <span className="text-secondary block text-[10px]">مدة الصلاحية:</span>
                        <span className="font-bold font-mono text-on-background">{form.access_days} يوم</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div>
                {wizardStep > 1 ? (
                  <button 
                    ...
                    className="px-5 py-2.5 rounded-xl text-xs font-bold text-secondary hover:bg-slate-100 transition duration-300"
                  >
                    السابق
                  </button>
                ) : (
              ...
              <div>
                {wizardStep < 3 ? (
                  <button 
                    ...
                    className="px-6 py-2.5 bg-primary hover:bg-primary-container text-white font-bold rounded-xl text-xs shadow-md shadow-primary/20 transition duration-300"
                  >
                    التالي
                  </button>
                ) : (
