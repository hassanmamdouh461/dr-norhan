...
export default function QuestionsPage() {
  ...
  return (
    <div className="space-y-6 select-none text-on-surface h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden min-h-0">
        ...
        <div className="lg:col-span-1 bg-white border border-slate-300 shadow-sm rounded-xl flex flex-col overflow-hidden">
          
          {/* Search and Filters */}
          <div className="p-4 border-b border-slate-300 bg-surface-container-low/50 space-y-3">
            <div className="relative">
              <input 
                placeholder="البحث في الأسئلة أو الطلاب..." 
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                className="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-300 focus:border-primary focus:ring-1 focus:ring-primary rounded-lg text-xs text-on-surface placeholder:text-outline focus:outline-none transition text-right shadow-sm" 
              />
              <span className="material-symbols-outlined absolute right-3 top-2 text-secondary text-lg">search</span>
            </div>
              ...
              <button 
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border transition ${
                  statusFilter === 'open' 
                    ...
                    : 'bg-white border-slate-300 text-on-surface-variant hover:bg-slate-50'
                }`}
              >
                بانتظار الرد
              </button>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-outline-variant/60 custom-scrollbar">
            {loading ? (
              ...
              <div 
                ...
                className={`p-4 cursor-pointer transition select-none relative hover:bg-slate-50/50 ${
                  selectedQuestionId === q.id ? 'bg-primary/5 hover:bg-primary/5' : ''
                }`}
              >
                {q.is_pinned === 1 && (
                  <span className="material-symbols-outlined text-[14px] text-primary absolute left-3 top-3" style={{ fontVariationSettings: "'FILL' 1" }}>
                    push_pin
                  </span>
                )}
            ...
            {hasMore && (
              <div className="p-3 text-center">
                <button
                  ...
                  className="w-full rounded-lg border border-outline-variant bg-white py-2 text-xs font-bold text-on-surface-variant transition hover:border-primary/40 hover:text-primary disabled:opacity-50"
                >
                  {loadingMore ? (
        ...
        <div className="lg:col-span-2 bg-white border border-slate-300 shadow-sm rounded-xl flex flex-col overflow-hidden h-full">
          
          {selectedQuestion ? (
            <div className="flex flex-col h-full overflow-hidden">
              
              {/* Detail Header */}
              <div className="p-4 border-b border-slate-300 bg-surface-container-low/50 flex justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button 
                    ...
                    className="p-2 text-on-surface-variant hover:text-primary hover:bg-slate-50 rounded-lg transition"
                  >
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-6 custom-scrollbar bg-slate-50/30">
                
                {/* Student's original question post */}
                <div className="bg-white border border-slate-300 rounded-xl p-5 space-y-3 shadow-sm max-w-[90%] md:max-w-[80%]">
                  <div className="flex justify-between items-center text-[10px] text-outline border-b border-slate-100 pb-2">
                  ...
                  <div className="text-xs text-on-surface leading-relaxed text-right">
                  ...
                  <div className="space-y-4">
                    {answers.map(ans => {
                      ...
                      return (
                        ...
                        >
                          <div className={`rounded-xl p-4 border text-xs leading-relaxed text-right ${
                            isAdmin 
                              ? 'bg-primary-fixed text-on-primary-fixed-variant border-primary/20 shadow-sm'
                              : 'bg-white border-outline-variant shadow-sm'
                          }`}>
                            <div className="flex justify-between items-center text-[9px] text-outline mb-2 border-b border-black/5 pb-1 gap-4">
              ...
              <div className="p-4 border-t border-slate-300 bg-surface-container-low/50 space-y-3 shrink-0">
                {/* Formatting Tools */}
                <div className="flex items-center gap-1 bg-white border border-slate-300 shadow-sm rounded-lg p-1.5">
                  <button 
                    onClick={() => insertFormat('**', '**')} 
                    title="نص عريض"
                    className="p-1 text-secondary hover:text-primary hover:bg-slate-50 rounded transition flex items-center justify-center"
                  >
                  ...
                  <div className="border-r border-slate-300 h-5 mx-1" />
                  
                  <button 
                    onClick={() => document.getElementById('image-attachment-input')?.click()} 
                    title="إرفاق صورة مع الإجابة"
                    disabled={uploadingImage}
                    className="p-1.5 text-secondary hover:text-primary hover:bg-slate-50 rounded transition flex items-center justify-center gap-1 shrink-0"
                  >
                ...
                {(attachedImageUrl || uploadingImage) && (
                  <div className="flex items-center gap-3 bg-white border border-outline-variant rounded-xl p-2.5 shadow-sm">
                    {uploadingImage ? (
                      ...
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-[10px] text-secondary">تم إرفاق صورة مع الإجابة</span>
                        </div>
                        <button 
                ...
                {/* Reply Input Box */}
                <div className="flex items-end gap-2 bg-white border border-slate-300 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary rounded-xl p-2.5 shadow-sm transition duration-300">
                  <textarea 
                    id="reply-textarea"
                    ...
                    className="flex-1 bg-transparent text-xs text-on-surface placeholder:text-outline outline-none resize-none text-right"
                  />
                  <button 
                    onClick={() => submitAnswer(selectedQuestion.id)}
                    disabled={submitting || loadingAnswers || (!answerText.trim() && !attachedImageUrl)}
                    className="px-5 py-3 bg-primary hover:bg-primary-container text-white disabled:opacity-40 disabled:pointer-events-none rounded-xl text-xs font-bold transition flex items-center gap-1.5 self-end shrink-0 shadow-ambient"
                  >
                    {submitting ? (
                      <span className="material-symbols-outlined animate-spin text-sm">sync</span>
                    ) : (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-10 bg-slate-50/20">
              <span className="material-symbols-outlined text-secondary/30 text-5xl mb-3 block">forum</span>
              <p className="text-sm font-bold text-secondary">الرجاء اختيار أحد الأسئلة من القائمة لاستعراض التفاصيل وإرسال الإجابة</p>
            </div>
          )}
