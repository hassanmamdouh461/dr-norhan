...
export default function DashboardHome() {
  ...
  useEffect(() => {
    const loadRole = async () => {
      ...
      try {
        const raw = typeof window !== 'undefined' ? localStorage.getItem('chemistry_dashboard_session') : null;
                    ...
                    {studentResults.length > 0 && (
                      <div className="max-h-40 divide-y divide-outline-variant overflow-y-auto rounded-lg border border-outline-variant">
                        {studentResults.map((student) => (
                          <button type="button" key={student.id} onClick={() => { setSelectedNoticeStudent(student); setStudentResults([]); }} className="flex w-full items-center justify-between px-3 py-2 text-right text-xs transition hover:bg-surface-container-low">
