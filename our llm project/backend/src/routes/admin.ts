...
admin.post('/assistants', requireRole('admin'), async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `INSERT INTO profiles (id, supabase_user_id, email, role, full_name, phone, status, max_devices, platform, created_at, updated_at)
       VALUES (?, ?, ?, 'assistant', ?, ?, 'active', 5, ?, ?, ?)`
    ).bind(assistantId, `assistant-uid-${assistantId}`, d.email, d.full_name, d.phone || '', platform, now, now),
    c.env.DB.prepare(
...
admin.post('/quizzes/:id/questions', requirePermission('can_manage_courses'), async (c) => {
  ...
  const id = generateId();
  await c.env.DB.prepare(
    `INSERT INTO quiz_questions (id, quiz_id, question_text, image_url, options_json, correct_option, score, sort_order, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`
  ).bind(
    id,
    quizId,
...
admin.patch('/quizzes/questions/:id', requirePermission('can_manage_courses'), async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE quiz_questions 
     SET question_text = ?, image_url = ?, options_json = ?, correct_option = ?, score = ?, sort_order = ?, updated_at = datetime('now')
     WHERE id = ?`
  ).bind(qText, imgUrl, optJson, corrOpt, scoreVal, sortVal, id).run();
