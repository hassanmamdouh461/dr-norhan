...
function seedProfile(id: string, role = 'assistant', platform = 'dr-physics') {
...
describe.each([
  { label: 'configured platform', configured: 'alhadaba-chemistry', platform: 'alhadaba-chemistry', foreign: 'dr-physics' },
  { label: 'default platform', configured: undefined, platform: 'dr-physics', foreign: 'alhadaba-chemistry' },
...
describe.each([undefined, 'alhadaba-chemistry'])('standalone grade sheet (platform: %s)', (configured) => {
  it('keeps unfinished/missing grades null, submitted zero 0, and totals submitted scores only', async () => {
    const platform = configured || 'dr-physics';
    seedProfile('student', 'student', platform);
